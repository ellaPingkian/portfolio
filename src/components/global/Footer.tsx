...
function Footer() {
  ...
  return (
    <div className='phone:gap-8 px-10  desktop:px-48 widescreen:px-48  w-screen pt-12 flex flex-col bg-accent bg-opacity-0'>
      <div className='flex flex-col'>
        <span className='text-[#C2C2C2] font-secondary text-md tracking-[0.07em]'>I'm</span>
        <div className='py-6 antialiased flex flex-col text-[195px] phone:text-[96px] tablet:text-[145px] font-main text-accent leading-[75%]'>
          <span>One <br/> Email Away</span>
        </div>
        <span className='text-[#C2C2C2] font-secondary text-base tracking-[0.07em] w-[55%] phone:w-full'>Discover how I turn intricate concepts into seamless user journeys. And let's collaborate to create user-centric designs that make an impact.</span>
      </div>

      <div className='flex flex-row pb-8 phone:pb-0 phone:flex-col-reverse phone:gap-8 '>

        <div className='w-full gap-10 phone:gap-2 flex justify-start items-end md:justify-center phone:justify-between'>
          { socials.map((item) => {
            return (
              <Link to={item.path} target="_blank" className='h-[45px] w-[45px] phone:scale-90 bg-accent opacity-25 text-white rounded-full hover:opacity-100 flex items-center justify-center hover:outline-dashed hover:outline-2 hover:outline-offset-8 hover:scale-110 hover:outline-accent ease-in-out duration-300'>
                {item.icon}
              </Link>
            )
          })}
        ...
        <Link to='mailto:ellpingkian@gmail.com' className='flex flex-row w-[50%] phone:w-full phone:justify-center items-end gap-8 text-[#C2C2C2] hover:text-accent ease-in-out duration-300'>
      ...
      <div className='flex justify-center h-12 items-center gap-4 py-8 phone:py-2 '>
